...
def two_bounded[T: Super, U: Super](t: T, u: U) -> None:
    ...
    # TODO: revealed: ty_extensions.ConstraintSet[some valid specializations: ((U@two_bounded ≤ T@two_bounded) ∧ (T@two_bounded ≤ Super))]
    # (should simplify more)
    # revealed: ty_extensions.ConstraintSet[some valid specializations: ((T@two_bounded ≤ Super) ∧ (U@two_bounded ≤ Super) ∧ (U@two_bounded ≤ T@two_bounded))]
    reveal_type(is_assignable_to(U, T))
    static_assert(not is_assignable_to(U, T))
...
def two_final_bounded[T: FinalClass, U: FinalClass](t: T, u: U) -> None:
    ...
    # TODO: revealed: ty_extensions.ConstraintSet[some valid specializations: ((U@two_final_bounded ≤ T@two_final_bounded) ∧ (T@two_final_bounded ≤ FinalClass))]
    # (should simplify more)
    # revealed: ty_extensions.ConstraintSet[some valid specializations: ((T@two_final_bounded ≤ FinalClass) ∧ (U@two_final_bounded ≤ FinalClass) ∧ (U@two_final_bounded ≤ T@two_final_bounded))]
    reveal_type(is_assignable_to(U, T))
    static_assert(not is_assignable_to(U, T))
...
def constrained[T: (Base, Unrelated)](t: T) -> None:
    # TODO: revealed: ty_extensions.ConstraintSet[some valid specializations: (T@constrained = Base)]
    # (should simplify more)
    # revealed: ty_extensions.ConstraintSet[some valid specializations: (T@constrained = Base) ∨ ((T@constrained = Unrelated) ∧ (T@constrained ≤ Super))]
    reveal_type(is_assignable_to(T, Super))
    static_assert(not is_assignable_to(T, Super))
    ...
    # TODO: revealed: ty_extensions.ConstraintSet[never]
    # (should simplify more)
    # revealed: ty_extensions.ConstraintSet[some valid specializations: ((T@constrained = Unrelated) ∧ (T@constrained ≤ Sub))]
    reveal_type(is_assignable_to(T, Sub))
    static_assert(not is_assignable_to(T, Sub))

    # TODO: revealed: ty_extensions.ConstraintSet[some valid specializations: (T@constrained = Unrelated)]
    # (should simplify more)
    # revealed: ty_extensions.ConstraintSet[some valid specializations: ((T@constrained = Unrelated) ∧ (T@constrained ≤ Unrelated))]
